...
def potential_energy_direct(pos: Array, mass: float, cfg: SimConfig) -> float:
    ...
    Row-blocked to keep memory O(block*N) instead of O(N^2): the naive
    (N,N,3) difference array is 6 GB at N=16384 and OOM-kills small-RAM
    instances. Blocking gives a bit-identical result (each unordered pair
    summed once, via 0.5 * full off-diagonal sum) at ~block/N of the memory.
    """
    if cfg.model == "pm_periodic":
        return float("nan")
    ...
    for i0 in range(0, n, block):
        i1 = min(i0 + block, n)
        dx = pos[i0:i1, None, :] - pos[None, :, :]         # (b, N, 3)
        if periodic:
            dx = min_image(dx, cfg.box_size)
        r2 = np.sum(dx * dx, axis=-1) + eps2               # (b, N)
        rows = np.arange(i1 - i0)
        r2[rows, i0 + rows] = np.inf                       # exclude self (1/inf=0)
        total += float(np.sum(1.0 / np.sqrt(r2)))
    # full off-diagonal double-counts each pair; halve to match triu(k=1)
    return float(-cfg.G * mass * mass * 0.5 * total)
